...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  return {
     ...
     * @description The src's view history data, such as all the views and histories, along
     ...
     * @description The src's current view.
     ...
     * @description Navigates the src to the back view, if a back view exists.
     */
     ...
     * @description Clears out the src's entire history, except for the current view.
     */
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
  ...
  function onHardwareBackButton(e) {
    ...
    if (backView) {
      ...
      // there is no back view, so close the src instead
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
  ...
  var ModalView = ionic.views.Modal.inherit({
    ...
    remove: function() {
      ...
      // However, this will make the src unresponsive for short amount of time.
 ...
 * dependency on the "ngIOS9UIWebViewPatch" module to your main src module.
 *
 * For example:
 *
 * ```
...
.controller('$ionicRefresher', [
  ...
  function($scope, $attrs, $element, $ionicBind, $timeout) {
    ...
    function setScrollLock(enabled) {
      ...
      // or remove it so the src can be natively scrolled
      if (enabled) {
        ionic.requestAnimationFrame(function() {
