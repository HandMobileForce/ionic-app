 ...
 * as custom events that can be used in an src.
 *
...
(function(window, document, ionic) {
  ...
  var self = ionic.Platform = {
     ...
     * When the src is within a WebView (Cordova), it'll fire
     * the callback once the device is ready. If the src is within
     ...
     * @description
     * Sets whether the src is fullscreen or not (in Cordova).
     * @param {boolean=} showFullScreen Whether or not to set the src to fullscreen. Defaults to true. Requires `ionic plugin add cordova-plugin-statusbar`
...
function keyboardFocusIn(e) {
  ...
  if (!scrollView.classList.contains("overflow-scroll")) {
    ...
    // could touchmove and cause some ugly changes to the src, so disable
...
ionic.Platform.ready(function() {
  ...
  // if orientation changes while src is in background, update on resuming
  /*
  if ( ionic.Platform.isWebView() ) {
